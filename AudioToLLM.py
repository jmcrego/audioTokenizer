...
class AudioToLLM(torch.nn.Module):
    ...
    def __init__(self, config, device, dtype, is_infer=False):
        ...
        self.audio_token_id = config['llm']['token_map'][self.audio_token] #or: self.tokenizer.convert_tokens_to_ids(self.audio_token)
    ...
    #         f"prompt norm={prompt_embs.norm(dim=-1).mean():.2f}"
    #     )
    ...
    #     inputs_embeds[batch_idx.unsqueeze(1).expand_as(audio_idx)[audio_valid], audio_idx[audio_valid]] = proj_embs[audio_valid]
    #     attention_mask[batch_idx.unsqueeze(1).expand_as(audio_idx)[audio_valid], audio_idx[audio_valid]] = 1

    #     # ----------------------------
    #     # 5) Copy prompt embeddings
    #     # ----------------------------
    #     dest_prompt_pos = audio_lens.unsqueeze(1) + torch.arange(T_prompt, device=device).unsqueeze(0)
    #     batch_idx_prompt = batch_idx.unsqueeze(1).expand_as(dest_prompt_pos)
    #     inputs_embeds[batch_idx_prompt, dest_prompt_pos] = prompt_embs
    #     attention_mask[batch_idx_prompt, dest_prompt_pos] = 1
    ...
    #     # ----------------------------
    #     # 8) Decode generated tokens (no slicing!)
    #     # ----------------------------
    #     texts = self.tokenizer.batch_decode(outputs, skip_special_tokens=True)

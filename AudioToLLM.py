...
class AudioToLLM(torch.nn.Module):
    ...
    def __init__(self, config, device, dtype, is_infer=False):
        ...
        self.llm = LLM(config['llm'], config['lora'], config['embeddings'])
        ...
        self.projector = Projector(
            ...
            llm_embedding_dim=self.llm.model.config.hidden_size
        )
        ...
        self.llm.model.to(device=device, dtype=dtype)      #float16/bfloat16 is for memory efficiency
        ...
        logger.info(f"LLM: {next(self.llm.model.parameters()).dtype} on {next(self.llm.model.parameters()).device}")
    ...
    def format_batch(self, audio_paths, prompt_ids, target_ids=None, pt_paths=None, offsets=None):
        ...
        llm_dtype = next(self.llm.model.parameters()).dtype
        B = prompt_ids.size(0)
...
#            logger.info(f"labels.shape {labels.shape}")
    ...
    def generate(
        ...
        #     eos_probs.append(probs[:, self.llm.tokenizer.eos_token_id]) # [B, i]
        # eos_probs = torch.stack(eos_probs, dim=1)  # [B, T_new]
        # mean_eos_prob = eos_probs.mean()
        # max_eos_prob = eos_probs.max()
        # logger.info(f"Mean EOS prob: {mean_eos_prob.item():.6f} Max EOS prob: {max_eos_prob.item():.6f}")
    ...
    def summary(self):
        ...
        # -----------------------------
        llm_lora_total = sum(
            p.numel() for n, p in self.llm.model.named_parameters() if "lora" in n.lower()
        )
        llm_lora_trainable = sum(
            p.numel() for n, p in self.llm.model.named_parameters() if "lora" in n.lower() and p.requires_grad
        )

        # -----------------------------
        # LLM Embeddings (special tokens)
        # -----------------------------
        if getattr(self.llm, "new_vocab_size", None) is not None:
            V_old = self.llm.original_vocab_size
            V_new = self.llm.new_vocab_size
            D = self.llm.model.get_input_embeddings().weight.shape[1]

            llm_emb_total = 2 * V_new * D  # input + output embeddings
            llm_emb_trainable = 2 * (V_new - V_old) * D
            llm_emb_frozen = llm_emb_total - llm_emb_trainable
        else:
            llm_emb_total = llm_emb_trainable = llm_emb_frozen = 0

        # -----------------------------
        # LLM Backbone (rest)
        # -----------------------------
        llm_total = sum(p.numel() for p in self.llm.model.parameters())
        llm_frozen = sum(p.numel() for p in self.llm.model.parameters() if not p.requires_grad)

        # Backbone = LLM total - LoRA - embeddings
        llm_backbone_total = llm_total - llm_lora_total - llm_emb_total
        llm_backbone_trainable = 0  # LoRA + embeddings are counted separately
        llm_backbone_frozen = llm_backbone_total
        ...
        logger.info(
            ...
            f"{embedder_trainable:>15,} | "
            f"{embedder_total - embedder_trainable:>15,}"
        )
        logger.info(
            f"Projector       : {projector_total:>15,} | "
            f"{projector_trainable:>15,} | "
            f"{projector_total - projector_trainable:>15,}"
        )
        logger.info(
            f"LLM (LoRA)      : {llm_lora_total:>15,} | "
            f"{llm_lora_trainable:>15,} | "
            f"{llm_lora_total - llm_lora_trainable:>15,}"
        )
        logger.info(
            f"LLM (Embeddings): {llm_emb_total:>15,} | "
            f"{llm_emb_trainable:>15,} | "
            f"{llm_emb_frozen:>15,}"
        )
        logger.info(
            f"LLM (Backbone)  : {llm_backbone_total:>15,} | "
            f"{llm_backbone_trainable:>15,} | "
            f"{llm_backbone_frozen:>15,}"
        )
        logger.info("-" * 100)
        logger.info(
            f"TOTAL           : {total_params:>15,} | "
            f"{trainable_params:>15,} | "
            f"{frozen_params:>15,}"
        )

...
def audio2embeddings(embedder, data_path: str, max_audio_files: int = None, max_frames_file: int = None,  max_frames_total: int = None):
    ...
    for i, path in enumerate(audio_files):
        try:
            ...
            print(f"  {i+1}/{len(audio_files)} {os.path.basename(path)}: {emb.shape[0]} frames, dim {emb.shape[1]}")
    ...
    sample_rate = 16000
    stride = 320
    print(f"Total frames: {len(X)}, dim: {X.shape[1]}, time: {secs2human(len(X) * stride / sample_rate)}")
    ...
    if max_frames_total is not None and len(X) > max_frames_total:
        ...
        print(f"Subsampled to {len(X)} frames.")
...
def train_kmeans(embeddings: np.ndarray, k: int, max_iter=1000, batch_size=16384):
    ...
    print(f"Training MiniBatchKMeans: k={k}, dim={embeddings.shape[1]}")
...
if __name__ == "__main__":
    ...
    output = args.oname + f".{os.path.basename(args.model)}" + f".{args.k}.npy"
